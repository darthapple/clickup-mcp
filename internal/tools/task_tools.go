...
var taskFilterOptions = []mcp.ToolOption{
	...
	mcp.WithArray("assignees", mcp.WithStringItems(), mcp.Description("Filter by assignee user IDs")),
	mcp.WithArray("tags", mcp.WithStringItems(), mcp.Description("Filter by tag names")),
	mcp.WithString("custom_fields_json", mcp.Description("Raw JSON array of custom field filters, ClickUp's custom_fields query param")),
}
...
func RegisterTaskTools(s *server.MCPServer, c *clickup.Client) {
	...
	s.AddTool(
		mcp.NewTool("clickup_create_task",
			...
			mcp.WithNumber("start_date", mcp.Description("Start date, Unix ms timestamp")),
	...
	s.AddTool(
		mcp.NewTool("clickup_get_bulk_time_in_status",
			...
			mcp.WithArray("task_ids", mcp.Required(), mcp.WithStringItems(), mcp.Description("Task IDs")),
		),
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	s.AddTool(
		mcp.NewTool("clickup_add_task_link",
			...
			mcp.WithString("task_id", mcp.Required(), mcp.Description("Task ID")),
			mcp.WithString("links_to", mcp.Required(), mcp.Description("ID of the task to link to")),
		),
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			taskID, err := req.RequireString("task_id")
			if err != nil {
	...
	s.AddTool(
		mcp.NewTool("clickup_remove_task_link",
			...
			mcp.WithString("task_id", mcp.Required(), mcp.Description("Task ID")),
			mcp.WithString("links_to", mcp.Required(), mcp.Description("ID of the linked task")),
		),
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			taskID, err := req.RequireString("task_id")
			if err != nil {
