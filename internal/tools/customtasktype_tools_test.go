...
func TestClickupListCustomTaskTypes(t *testing.T) {
	t.Run("argument wiring", func(t *testing.T) {
		var gotMethod, gotPath string
		c, _ := newTestClient(t, func(w http.ResponseWriter, r *http.Request) {
			gotMethod = r.Method
			gotPath = r.URL.Path
			_, _ = w.Write([]byte(`{"custom_items":[{"id":1}]}`))
		})
		s := server.NewMCPServer("test", "1.0.0")
		RegisterCustomTaskTypeTools(s, c)
		res := callTool(t, s, "clickup_list_custom_task_types", map[string]any{"team_id": "123"})
		if res.IsError {
		...
		if gotPath != "/team/123/custom_item" {
			t.Errorf("path = %q, want /team/123/custom_item", gotPath)
		}
	})
	...
	t.Run("defaults team_id from config", func(t *testing.T) {
		...
		s := server.NewMCPServer("test", "1.0.0")
		RegisterCustomTaskTypeTools(s, c)
		res := callTool(t, s, "clickup_list_custom_task_types", map[string]any{})
		if res.IsError {
			t.Fatalf("IsError = true, want false: %s", textOf(t, res))
		}
		if gotPath != "/team/999/custom_item" {
			t.Errorf("path = %q, want /team/999/custom_item (default team id)", gotPath)
		}
	})
